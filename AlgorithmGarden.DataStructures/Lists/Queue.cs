...
namespace AlgorithmGarden.DataStructures.Lists
{
    ...
    //一种方式是拒绝任务请求
    //一种是阻塞的方式，将请求排队，等到有空闲线程时，取出排队请求继续处理
    //每个排队的请求存储起来，先进先出
    //存储排队请求的队列可以是链表队列，链表队列是无界队列unbounded queue，可能会导致过多的请求在排队等候，处理的响应时间过长
    //基于数组的队列是bounded queue,队列大小有限，线程池中排列的请求超出容量的时候，接下来的请求会被拒绝。设置一个合理的队列大小非常有讲究。
